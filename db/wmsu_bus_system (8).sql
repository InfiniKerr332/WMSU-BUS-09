...
--
-- Host: 127.0.0.1
-- Generation Time: Dec 14, 2025 at 06:53 AM
...
-- Functions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `get_full_name` (`p_first_name` VARCHAR(50), `p_middle_name` VARCHAR(50), `p_last_name` VARCHAR(50)) RETURNS VARCHAR(152) CHARSET utf8mb4 COLLATE utf8mb4_general_ci DETERMINISTIC BEGIN
...
(79, 'wmsubussystem@gmail.com', 'New Bus Reservation - Driver Assignment Needed', 'sent', NULL, '2025-12-14 04:16:44');
...
(34, 15, 'reservation', 'Reservation Submitted', 'Your reservation for December 25, 2025 is pending admin approval', 'student/my_reservations.php?view=27', 0, '2025-12-14 04:16:40'),
(35, 4, 'reservation', 'New Reservation from Kerr', 'December 25, 2025 to 6 7 avenue street - Awaiting approval', 'admin/reservations.php?view=27', 0, '2025-12-14 04:16:40');
...
(27, 15, 1, NULL, '6 7 meeting', '6 7 avenue street', '2025-12-25', '2025-12-27', '13:11:00', '12:12:00', 30, 'pending', NULL, NULL, NULL, 0, '2025-12-14 04:16:40');
...
(15, NULL, NULL, NULL, 'Kerr Xandrex Chua Zaragoza', 'ae202401442@wmsu.edu.ph', NULL, 'BSIT', 'Teacher', '$2y$10$egi2LxRb0aVykcYTsLOLzelaGHlNyqo/ZsfJL6Hg53XsOqWh7gp/O', 'uploads/employee_ids/emp_front_1765640172_693d87ec9b69f.jpg', 'uploads/employee_ids/emp_back_1765640172_693d87ec9d20a.jpg', 'approved', 1, '2025-12-13 15:45:00', NULL, '2025-12-14 23:36:12', 4, '2025-12-13 15:47:34', NULL, 'teacher', '2025-12-13 15:36:12'),
(17, NULL, NULL, NULL, 'Test Teacher User', 'ae20242831@wmsu.edu.ph', NULL, 'CCS', 'Teacher', '$2y$10$2QzlHxcObiLkyKPuFv3oQunxAY9wfREBieNBPCr733vkTqsmsUHcK', NULL, NULL, 'approved', 1, '2025-12-14 02:48:32', NULL, NULL, NULL, NULL, NULL, 'teacher', '2025-12-14 02:48:32');

-- --------------------------------------------------------

--
-- Stand-in structure for view `users_with_full_name`
-- (See below for the actual view)
--
CREATE TABLE `users_with_full_name` (
`id` int(11)
...
,`role` enum('student','employee','teacher','admin')
,`account_status` enum('pending','approved','rejected')
,`email_verified` tinyint(1)
,`email_verified_at` timestamp
,`verification_token` varchar(64)
,`verification_expires` datetime
,`employee_id_image` varchar(255)
,`employee_id_back_image` varchar(255)
,`approved_by_admin` int(11)
...
--
-- Structure for view `users_with_full_name`
--
DROP TABLE IF EXISTS `users_with_full_name`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `users_with_full_name`  AS SELECT `users`.`id` AS `id`, `users`.`first_name` AS `first_name`, `users`.`middle_name` AS `middle_name`, `users`.`last_name` AS `last_name`, concat_ws(' ',`users`.`first_name`,nullif(`users`.`middle_name`,''),`users`.`last_name`) AS `name`, `users`.`email` AS `email`, `users`.`password` AS `password`, `users`.`contact_no` AS `contact_no`, `users`.`department` AS `department`, `users`.`position` AS `position`, `users`.`role` AS `role`, `users`.`account_status` AS `account_status`, `users`.`email_verified` AS `email_verified`, `users`.`email_verified_at` AS `email_verified_at`, `users`.`verification_token` AS `verification_token`, `users`.`verification_expires` AS `verification_expires`, `users`.`employee_id_image` AS `employee_id_image`, `users`.`employee_id_back_image` AS `employee_id_back_image`, `users`.`approved_by_admin` AS `approved_by_admin`, `users`.`approved_at` AS `approved_at`, `users`.`rejection_reason` AS `rejection_reason`, `users`.`created_at` AS `created_at` FROM `users` ;
...
ALTER TABLE `drivers`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `idx_unique_driver_contact` (`contact_no`),
  ADD UNIQUE KEY `idx_unique_driver_license` (`license_no`),
...
ALTER TABLE `reservations`
  ...
  ADD KEY `idx_reservations_bus_date_status` (`bus_id`,`reservation_date`,`return_date`,`status`),
  ADD KEY `idx_reservations_driver_date_status` (`driver_id`,`reservation_date`,`return_date`,`status`),
  ADD KEY `idx_reservations_user_status` (`user_id`,`status`,`reservation_date`);
...
ALTER TABLE `users`
  ...
  ADD UNIQUE KEY `idx_unique_user_contact` (`contact_no`),
